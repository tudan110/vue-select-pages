      ...
      <div class="single-row" v-else>
        <div class="scroll">
          <div class="label-content">
            ...
            <template v-else>
              <div class="label-block" :style="{background:theme?theme:'#409eff'}"
                   v-for="(list, index) in selectData.slice(0, showCount)" :key="index">
    ...
    <div class="select-body" ref="selectOptionRef" :class="{'hide': !show}" :style="dropStyle">
      ...
      <div class="scroll-body" v-if="optionData.length>0">
        <div class="select-option" :class="{'selected': radio && selectIds.indexOf(list[prop.value]) != -1}"
             v-for="(list, index) in optionData.slice(isPage && !remote?(page-1)*pageSize:0, (isPage && !remote?(page*pageSize):optionData.length))"
             :key="index" @click="selectSingle(list)">
          ...
          <label class="select-checkbox" :style="{'background-color':theme,'border-color':theme}" v-else>
            <span class="select-checkbox-input"
                  :class="{'is-checked': selectIds.indexOf(list[prop.value]) != -1}"></span>
            <slot name="title" v-bind:row="list">{{ texts(list) }}</slot>
          </label>
        </div>
